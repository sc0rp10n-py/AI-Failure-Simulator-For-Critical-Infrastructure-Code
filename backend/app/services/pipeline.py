...
def start_analysis(project_id: int, reanalyze: bool = False) -> str:
...
def _run_pipeline(job_id: str, reanalyze: bool) -> None:
    ...
    try:
        ...
        _advance(db, run, "extraction_complete", "Extracted project tree ready for scan")
        _advance(db, run, "dependency_scan_complete", "Mapping service and dependency graph")

        analysis = analyze_project(root)
        project.framework = analysis.framework
        project.dependency_count = len(analysis.dependencies[0].get("nodes", [])) if analysis.dependencies else 0
        db.add(
            RiskReport(run_id=run.id, payload=analysis.to_dict())
        )
        _advance(db, run, "risk_analysis_complete", f"Risk score {analysis.risk_score():.0f} ({analysis.severity_label()})")
        ...
        _advance(db, run, "failure_generation_complete", f"Generated {len(scenarios)} failure scenarios")

        telemetry = run_simulation(root, analysis, scenarios, output_dir)
        for scenario in scenarios:
            existing = (
        ...
        _advance(db, run, "simulation_complete", "Executed autonomous failure injections")
        ...
        db.add(bundle)
        for entry in telemetry.get("logs", []):
            pass
        _log_simulation_entries(db, run.id, output_dir)
        _advance(db, run, "telemetry_complete", "Telemetry captured")
        ...
        _advance(db, run, "ai_analysis_complete", "AI resilience report generated")
        db.commit()
        logger.info("Pipeline complete", extra={"job_id": job_id, "event": "pipeline_complete"})
    except Exception as exc:
        logger.info("Pipeline failed", extra={"job_id": job_id, "event": "pipeline_failed", "error": str(exc)})
        run = db.query(AnalysisRun).filter_by(job_id=job_id).first()
        if run:
            run.status = "failed"
            run.summary = str(exc)
...
def _advance(db, run: AnalysisRun, stage: str, message: str) -> None:
